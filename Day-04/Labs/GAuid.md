...
Timing (≈3 hours)
0:00–0:20 — Threat model warm-up. Walk the deck's five LLM threats (injection, jailbreak, model inversion/extraction, hallucination) and the guardrail taxonomy (input / output / policy / data / HITL). Land the distinction on slide 60: monitoring = what failed; observability = why.
0:20–1:10 — Security layer. Students implement Steps 1–4 (input guardrail, PII redaction, output guardrail, tool/HITL gate). Have them run pentest early — it will FAIL until the guardrails exist, which is the point.
...
1:40–2:30 — Monitoring layer. Steps Obs-0/1/2 + the entrypoint glue (Step 5) and FastAPI (Step 7). They should see JSON logs per node, /metrics with block-rate and p95 latency, and a /report that returns 422 on a blocked prompt (a blocked attack is a normal event, not a crash).
2:30–3:00 — Optional real observability + wrap. Anyone with a Langfuse key flips TRACE=1 and sees the thought trace online (deck slide 59). Close with "what's still missing?" → auth, rate limiting, persistent audit trail → which is exactly the capstone.
...
Add one new attack category to ATTACK_SUITE (e.g. base64-encoded injection) and defend against it.
Add a policy guardrail (deck slide 31): block topics outside an allowlist of domains.
Wire the tool_gate into a real high-risk action and prove the HITL approver stops it.
Export /metrics in Prometheus text format instead of JSON (deck slide 59) and scrape it.