...
class Settings:
    ...
    log_level: str = "INFO"
    report_style: str = "formal"        # YOUR TURN (Stage 2) — "formal" | "casual"
    ...
    def from_env(cls) -> "Settings":
        ...
        return cls(
            ...
            report_style=os.getenv("REPORT_STYLE", cls.report_style),  # YOUR TURN (Stage 2)
        )
...
class FakeChatModel:
    ...
    def invoke(self, prompt: str):
        ...
        # Extract topic so the fake draft passes the topic-presence guardrail
        ...
        return FakeResponse(
            ...
            f"BODY\nThe main findings about {topic} indicate steady growth, meaningful adoption "
            "across industries, and a set of open challenges around governance, integration, "
...
def call_llm(prompt: str, node: str, state: ReportState) -> str:
    ...
    for attempt in range(1, attempts + 1):
        start = time.time()
        try:
            # YOUR TURN (Stage 1) — flaky demo: 30% random failure so retries fire
            if STAGE >= 1 and random.random() < 0.3:
...
INJECTION_PATTERNS = [
    ...
    r"you are now",
    r"pretend to be",
    r"act as",                             # YOUR TURN (Stage 4) — catches "act as DAN" etc.
]
...
def validate_report(report: str, topic: str = "") -> None:
    ...
    # YOUR TURN (Stage 4) — report must actually mention the requested topic
    if topic and topic.lower() not in report.lower():
        raise ValueError(f"Output guardrail: report does not mention the topic '{topic}'.")
...
def write_node(state: ReportState) -> ReportState:
    ...
    draft = call_llm(
        ...
        f"\n\nWrite in a {settings.report_style} tone.",   # YOUR TURN (Stage 2)
        node="write",
        state=state,
    )
    state["draft"] = draft
...
def generate_report(topic: str) -> ReportState:
    run_start = time.time()    # YOUR TURN (Stage 3) — used for total_duration_s below
    state: ReportState = {
        "topic": topic,
    ...
    if STAGE >= 4 and "draft" in final:
        validate_report(final["draft"], final.get("topic", ""))   # YOUR TURN (Stage 4)
    ...
    if STAGE >= 3:
        log_event(
            ...
            total_duration_s=round(time.time() - run_start, 2),  # YOUR TURN (Stage 3)
        )
    return final
...
# YOUR TURN (Stage 5) — in-process counters; see /metrics discussion below
_metrics = {"total_runs": 0, "total_cost_usd": 0.0}
...
def create_app():
    ...
    def report(req: ReportRequest):
        ...
        _metrics["total_runs"] += 1                                    # YOUR TURN (Stage 5)
        _metrics["total_cost_usd"] = round(                            # YOUR TURN (Stage 5)
            _metrics["total_cost_usd"] + (result.get("cost_usd") or 0.0), 6
        )
        return {
    ...
    @app.get("/metrics")                                               # YOUR TURN (Stage 5)
    def get_metrics():
        # NOTE: this dict lives in ONE process — three replicas would each
        # have their own copy. A real system stores these in Redis or a DB.
        return _metrics
